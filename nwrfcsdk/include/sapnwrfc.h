...
typedef short          RFC_CDAY     ;       ///< Calendar day: Day and month
...
extern "C"
{
	...
	/**
	 * \brief  Sets the global CPIC pong timeout for the keepalive.
	 *
	 ...
	 *
	 * \param timeout The pong timeout in seconds. Valid values are between 10 and 3600 seconds, or 0 to deactivate the timeout.
	 * \param errorInfo Pointer to an RFC_ERROR_INFO structure to receive detailed error information in case of failure.
	 * \return RFC_OK if the operation was successful, or an appropriate RFC_RC error code otherwise.
	 */
	DECL_EXP RFC_RC SAP_API RfcSetCpicPongTimeout(unsigned timeout, RFC_ERROR_INFO* errorInfo);
    ...
    */
	...
	/**
	* \brief  Sets the global idle time interval of a Websocket connection in seconds after which
	...
	* \return RFC_RC
	*/
	DECL_EXP RFC_RC SAP_API RfcUTF8ToSAPUC_CCE(const RFC_BYTE* utf8, unsigned utf8Length, SAP_UC* sapuc, unsigned* sapucSize, unsigned* resultLength, unsigned short onCCE, SAP_UINT substitute, RFC_ERROR_INFO* errorInfo);
	...
	* \return RFC_OK or RFC_BUFFER_TOO_SMALL
	*/
	DECL_EXP RFC_RC SAP_API RfcSAPUCToUTF8_CCE(const SAP_UC* sapuc, unsigned sapucLength, RFC_BYTE* utf8, unsigned* utf8Size, unsigned* resultLength, unsigned short onCCE, SAP_UINT substitute, RFC_ERROR_INFO* errorInfo);
	...
	*/
	...
	/**
	 * \brief  Removes and deletes a function description from the cache for the specified R/3 System.
	 * \ingroup repository
     * 
	...
	/**
	 * \brief  Removes and deletes a type description from the cache.
	 * \ingroup repository
	 *
	 * Already existing function modules will use the old description, after the description was removed and deleted
	 * from the cache.
    ...
    /**
	 * \brief  Removes and deletes a class description from the cache for the specified R/3 System.
	 * \ingroup repository
     *
	 * Already existing function modules will use the old description, after the description was removed and deleted
	 * from the cache.
